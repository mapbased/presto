        ...
        extends AbstractTestHiveFileFormats
{
            ...
            throws Exception
    {
        List<TestColumn> testColumns = ImmutableList.copyOf(filter(TEST_COLUMNS, new Predicate<TestColumn>()
        {
            ...
            {
                // empty string column in ORC is broken
                if (testColumn.getName().equals("t_empty_string")) {
                    return false;
                }
                // orc vector does not work with map or list (and we haven't implemented struct yet)
                ...
                // also vector does not work with binary
                return objectInspector.getPrimitiveCategory() != PrimitiveCategory.BINARY;
            }
        }));
