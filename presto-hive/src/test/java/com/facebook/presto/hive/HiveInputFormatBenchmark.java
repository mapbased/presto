...
//        logging.setLevel("parquet", "OFF");
    ...
    public static DataSize writeLineItems(File outputFile, HiveOutputFormat<?, ?> outputFormat, SerDe serDe, String compressionCodec, int rowsPerNull)
            throws Exception
    {
        FSRecordWriter recordWriter = createRecordWriter(LINE_ITEM_COLUMNS, outputFile, outputFormat, compressionCodec);
    ...
    public static FSRecordWriter createRecordWriter(List<HiveColumn> columns, File outputFile, HiveOutputFormat<?, ?> outputFormat, String compressionCodec)
            throws Exception
    {
        JobConf jobConf = new JobConf();
        ReaderWriterProfiler.setProfilerOptions(jobConf);
