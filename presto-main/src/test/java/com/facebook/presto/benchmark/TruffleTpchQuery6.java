...
import com.facebook.presto.sql.truffle.FilterPageNode;
import com.facebook.presto.sql.truffle.FilterRowNode;
import com.facebook.presto.sql.truffle.MyArguments;
import com.facebook.presto.sql.truffle.SourcePageNode;
import com.facebook.presto.sql.truffle.SourceRowNode;
import com.facebook.presto.sql.truffle.SumPageNode;
import com.facebook.presto.sql.truffle.SumRowNode;
import com.facebook.presto.sql.truffle.TpchQuery6PageNode;
import com.facebook.presto.sql.truffle.TpchQuery6RowNode;
...
import static com.facebook.presto.sql.truffle.TpchQuery6RowNode.executeTpchQuery6;
import static com.facebook.presto.util.Threads.daemonThreadsNamed;
import static com.google.common.base.Charsets.UTF_8;
import static com.google.common.base.Preconditions.checkState;
import static java.util.concurrent.Executors.newCachedThreadPool;

public class TruffleTpchQuery6
        extends AbstractTruffleBenchmark
{
    private CallTarget function;

    public TruffleTpchQuery6(ExecutorService executor, TpchBlocksProvider tpchBlocksProvider)
    {
        super(executor, tpchBlocksProvider, "truffle_tpch_query_6", 0, 500);
    }
    ...
    protected Runnable createRootNode(TaskContext taskContext)
    {
        ...
        Operator operator = alignmentOperator.createOperator(driverContext);

        if (function == null) {
            if (false) {
                SourceRowNode sourceRowNode = new SourceRowNode();
                FilterRowNode filterRowNode = new FilterRowNode(sourceRowNode);
                SumRowNode sumRowNode = new SumRowNode(filterRowNode);
                function = Truffle.getRuntime().createCallTarget(sumRowNode);
            }

            if (false) {
                SourcePageNode sourcePageNode = new SourcePageNode();
                FilterPageNode filterPageNode = new FilterPageNode(sourcePageNode);
                SumPageNode sumPageNode = new SumPageNode(filterPageNode);
                function = Truffle.getRuntime().createCallTarget(sumPageNode);
            }

            if (true) {
                TpchQuery6RowNode sumPageNode = new TpchQuery6RowNode();
                function = Truffle.getRuntime().createCallTarget(sumPageNode);
            }

            if (false) {
                TpchQuery6PageNode sumPageNode = new TpchQuery6PageNode();
                function = Truffle.getRuntime().createCallTarget(sumPageNode);
            }
        }

        final MyArguments arguments = new MyArguments(operator);
...
//                function.call(arguments);
                executeTpchQuery6(arguments.getSourceOperator());
            }
        };
    }
    ...
    public static void main(String[] args)
    {
        ...
        new TruffleTpchQuery6(executor, DEFAULT_TPCH_BLOCKS_PROVIDER).runBenchmark(
