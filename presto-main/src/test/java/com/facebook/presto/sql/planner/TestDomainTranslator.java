...
public class TestDomainTranslator
{
    ...
    public void testRoundTrip()
            throws Exception
    {
        Map<Symbol, Domain<?>> domainMap = ImmutableMap.<Symbol, Domain<?>>builder()
                .put(A, Domain.single(1L))
                .put(B, Domain.onlyNull(Double.class))
                .put(C, Domain.notNull(String.class))
                .put(D, Domain.single(true))
                .put(E, Domain.single(2L))
                .put(F, Domain.create(SortedRangeSet.of(Range.lessThanOrEqual(1.1), Range.equal(2.0), Range.range(3.0, false, 3.5, true)), true))
                .put(G, Domain.create(SortedRangeSet.of(Range.lessThanOrEqual("2013-01-01"), Range.greaterThan("2013-10-01")), false))
                .build();


        ExtractionResult result = fromPredicate(toPredicate(domainMap), TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), domainMap);
    }
            ...
            throws Exception
    {
        ...
        Assert.assertEquals(simplify(toPredicate(domainMap)), FALSE_LITERAL);
    }
            ...
            throws Exception
    {
        ...
        ExtractionResult result = fromPredicate(toPredicate(domainMap), TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>builder()
                .put(A, Domain.single(1L))
                .put(B, Domain.onlyNull(Double.class))
                .put(C, Domain.notNull(String.class))
                .build());
    ...
    public void testToPredicate()
            throws Exception
    {
        Map<Symbol, Domain<?>> domainMap;

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.notNull(Long.class));
        Assert.assertEquals(toPredicate(domainMap), isNotNull(A));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.onlyNull(Long.class));
        Assert.assertEquals(toPredicate(domainMap), isNull(A));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.none(Long.class));
        Assert.assertEquals(toPredicate(domainMap), FALSE_LITERAL);

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.all(Long.class));
        Assert.assertEquals(toPredicate(domainMap), TRUE_LITERAL);

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.greaterThan(1L)), false));
        Assert.assertEquals(toPredicate(domainMap), greaterThan(A, longLiteral(1L)));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.greaterThanOrEqual(1L)), false));
        Assert.assertEquals(toPredicate(domainMap), greaterThanOrEqual(A, longLiteral(1L)));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(1L)), false));
        Assert.assertEquals(toPredicate(domainMap), lessThan(A, longLiteral(1L)));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.range(0L, false, 1L, true)), false));
        Assert.assertEquals(toPredicate(domainMap), and(greaterThan(A, longLiteral(0L)), lessThanOrEqual(A, longLiteral(1L))));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThanOrEqual(1L)), false));
        Assert.assertEquals(toPredicate(domainMap), lessThanOrEqual(A, longLiteral(1L)));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.single(1L));
        Assert.assertEquals(toPredicate(domainMap), equal(A, longLiteral(1L)));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.equal(1L), Range.equal(2L)), false));
        Assert.assertEquals(toPredicate(domainMap), in(A, ImmutableList.of(1L, 2L)));

        domainMap = ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(1L)), true));
        Assert.assertEquals(toPredicate(domainMap), or(lessThan(A, longLiteral(1L)), isNull(A)));
    }
    ...
    public void testFromUnknownPredicate()
            throws Exception
    {
        ExtractionResult result = fromPredicate(unprocessableExpression1(A), TYPES);
        Assert.assertTrue(result.getDomainMap().isEmpty());
        Assert.assertEquals(result.getRemainingExpression(), unprocessableExpression1(A));

        // Test the complement
        result = fromPredicate(not(unprocessableExpression1(A)), TYPES);
        Assert.assertTrue(result.getDomainMap().isEmpty());
            ...
            throws Exception
    {
        ...
        ExtractionResult result = fromPredicate(originalPredicate, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), and(unprocessableExpression1(A), unprocessableExpression2(A)));
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.range(1L, false, 5L, false)), false)));

        // Test complements
        originalPredicate = not(and(
                and(greaterThan(A, longLiteral(1L)), unprocessableExpression1(A)),
                and(lessThan(A, longLiteral(5L)), unprocessableExpression2(A))));
        result = fromPredicate(originalPredicate, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), originalPredicate);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of());
        ...
        result = fromPredicate(originalPredicate, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), originalPredicate);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.notNull(Long.class)));
    }
            ...
            throws Exception
    {
        ...
        result = fromPredicate(originalPredicate, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), originalPredicate);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.equal(1L), Range.equal(2L)), false)));
            ...
            throws Exception
    {
        ...
        result = fromPredicate(originalPredicate, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap().get(A), Domain.none(Long.class));

        originalPredicate = not(equal(A, longLiteral(1L)));
        result = fromPredicate(originalPredicate, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(1L), Range.greaterThan(1L)), false)));
    }
            ...
            throws Exception
    {
        ...
        ExtractionResult result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.greaterThan(2L)), false)));

        originalExpression = greaterThanOrEqual(A, longLiteral(2L));
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.greaterThanOrEqual(2L)), false)));

        originalExpression = lessThan(A, longLiteral(2L));
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(2L)), false)));

        originalExpression = lessThanOrEqual(A, longLiteral(2L));
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThanOrEqual(2L)), false)));

        originalExpression = equal(A, longLiteral(2L));
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.equal(2L)), false)));

        originalExpression = notEqual(A, longLiteral(2L));
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(2L), Range.greaterThan(2L)), false)));

        originalExpression = isDistinctFrom(A, longLiteral(2L));
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(2L), Range.greaterThan(2L)), true)));
            ...
            throws Exception
    {
        ...
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.onlyNull(Long.class)));
    }
            ...
            throws Exception
    {
        ...
        ExtractionResult result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(B, Domain.create(SortedRangeSet.of(Range.greaterThan(2.0)), false)));

        // C is a string column. Check that it can be compared.
        originalExpression = greaterThan(C, stringLiteral("test"));
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(C, Domain.create(SortedRangeSet.of(Range.greaterThan("test")), false)));
        ...
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.greaterThanOrEqual(3L)), false)));
        ...
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(3L)), false)));
        ...
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.none(Long.class)));
        ...
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(B, Domain.create(SortedRangeSet.of(Range.lessThanOrEqual(2.0)), false)));

        // C is a string column. Check that it can be compared.
        originalExpression = not(greaterThan(C, stringLiteral("test")));
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(C, Domain.create(SortedRangeSet.of(Range.lessThanOrEqual("test")), false)));
            ...
            throws Exception
    {
        ...
        ExtractionResult result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.single(1L)));
        ...
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(1L), Range.range(1L, false, 2L, false), Range.greaterThan(2L)), false)));
            ...
            throws Exception
    {
        ...
        ExtractionResult result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.range(1L, true, 2L, true)), false)));
        ...
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap(), ImmutableMap.<Symbol, Domain<?>>of(A, Domain.create(SortedRangeSet.of(Range.lessThan(1L), Range.greaterThan(2L)), false)));
        ...
        result = fromPredicate(originalExpression, TYPES);
        Assert.assertEquals(result.getRemainingExpression(), TRUE_LITERAL);
        Assert.assertEquals(result.getDomainMap().get(A), Domain.create(SortedRangeSet.of(Range.lessThan(1L)), false));
    }
    ...
    {
        ExpressionInterpreter optimizer = ExpressionInterpreter.expressionOptimizer(expression, new MetadataManager(), new Session(null, "", "", "", "", ""));
