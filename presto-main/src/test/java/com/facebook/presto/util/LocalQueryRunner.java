...
public class LocalQueryRunner
{
    ...
    {
        private AtomicReference<MaterializingOperator> materializingOperator = new AtomicReference<>();
    ...
    public List<Driver> createDrivers(@Language("SQL") String sql, OutputFactory outputFactory, TaskContext taskContext)
    {
        ...
        for (PlanNode sourceNode : subplan.getFragment().getSources()) {
            ...
            List<Partition> partitions = getPartitionsForScan(tableScan);
            DataSource dataSource = splitManager.getPartitionSplits(tableScan.getTable(), partitions);
